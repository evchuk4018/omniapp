...
export const chatService = {
  async resolveModel(conversationId: string, providerInput?: ProviderId, modelInput?: string): Promise<ModelSelection> {
    const conversation = await conversationRepo.get(conversationId);
    if (!conversation) throw new Error("Conversation not found.");
    if (providerInput && modelInput) return { provider: providerInput, modelTag: modelInput };
    if (conversation.activeModelTag) return { provider: normalizeProvider(conversation.provider), modelTag: conversation.activeModelTag };

    const defaults = await settingRepo.getDefaultModel();
    if (defaults.modelTag) return { provider: defaults.provider, modelTag: defaults.modelTag };

    const installed = await providerFactory.get("ollama").listInstalledModels();
    if (installed[0]) return { provider: "ollama", modelTag: installed[0].id };
